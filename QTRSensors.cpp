...
//NOTA ACTIVAR DEPURADOR O VISUALIZADOR DE DATOS GUARDADOS Y RESTAURADOS, QUITE LOS DOS SLASH en el siguiente renglon
//#define MICHAEL_DEBUG 1  // Activar log para saber si esta guardando y restaurando la calibración, escriba 1 para activar



// Constructor
EEPROMClassEx QTR_EEPROM; // Instanciar objeto para escritura y lectura de la EEMPROM
bool record=false;
...
bool QTRSensors::recordCalibration(){
  ...
  {                                                     //Gracias al uso de punteros por parte de la libreria se puede realizar lo siguiente
    record=(QTR_EEPROM.writeInt(i*2,calibratedMinimumOn[i]));  //Guardar los valores en la EEPROM cada 2 slots gracias a la libreria 
    record=(QTR_EEPROM.writeInt((i*2)+(_numSensors*2),calibratedMaximumOn[i])); //ya que en Arduino el int es de 2 Bytes y la EEMPROM almacena 1 byte por cada slot
  ...
  Serial.println("");
   Serial.println("Libreria modificada By Michael Vargas, prueba RW ( OK ) MOD 07/04/2015");
...
bool  QTRSensors::restoreCalibration(){  
 ...
 resetCalibration(); // Borrar valores old
     ...
     for (int i = 0; i < _numSensors; i++)
  {
     ...
     delay(2000);
     #endif
    calibratedMinimumOn[i] = QTR_EEPROM.readInt(i*2); //Asignar los valores guardados en la EEPROM en los valores de calibracion
    calibratedMaximumOn[i] = QTR_EEPROM.readInt((i*2)+(_numSensors*2));
...
bool QTRSensors::clearEEPROM(){
  for (int i = 0; i < (((_numSensors-1)*2)+(_numSensors*2)); i++){
    QTR_EEPROM.write(i, 0);
